...
| Seat counts per type (SWISS) | Per-type (72-320) | `aircraft_data.py` | SWISS actual cabin configs from [swiss.com fleet page](https://www.swiss.com/ch/en/discover/fleet). Long-haul class breakdowns are exact (F+J+W+Y = published total). Short-haul types use the published "passenger seats" total directly — seat map class breakdowns on these types are unreliable (swiss.com seat maps sometimes don't match published totals). |
| Seat counts per type (Edelweiss) | A320: 174, A340-300: 314, A350-900: 339 | `aircraft_data.py` | Edelweiss configs from [edelweissair.com fleet page](https://www.edelweissair.com/en/fly/our-fleet). Denser leisure configs than SWISS mainline. A340-300 has two configs (HB-JMC: 300 seats vs others: 314) — we use 314 (majority) because OpenSky does not provide registration, so per-aircraft differentiation is not yet possible. |
